...
while sample < total_runs:
    try:
        ...
        for train, test in kfold.split(training_data, training_labels):
            ...
            nn.fit(training_data[train], training_one_hot_encoded_labels[train], validation_split=0.2, epochs=50, batch_size=10)
...
np.savetxt('bcancernn.csv', mean_stats, fmt='%.3e', delimiter=',')
print(mean_stats)